...
export default function FahrlehrerPage() {
  const [records, setRecords] = useState<Fahrlehrer[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Fahrlehrer | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Fahrlehrer | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Fahrlehrer | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getFahrlehrer());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Fahrlehrer['fields']) {
    await LivingAppsService.createFahrlehrerEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Fahrlehrer['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateFahrlehrerEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteFahrlehrerEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Fahrlehrer"
      subtitle={`${records.length} Fahrlehrer im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('lizenzen')}>
                <span className="inline-flex items-center gap-1">
                  Lizenzen
                  {sortKey === 'lizenzen' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell>{Array.isArray(record.fields.lizenzen) ? record.fields.lizenzen.map((v: any) => v?.label ?? v).join(', ') : '—'}</TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={6} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Fahrlehrer. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
