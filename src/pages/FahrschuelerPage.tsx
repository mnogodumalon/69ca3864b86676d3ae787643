...
import type { Fahrschueler } from '@/types/app';
...
export default function FahrschuelerPage() {
  const [records, setRecords] = useState<Fahrschueler[]>([]);
  const [loading, setLoading] = useState(true);
  const [search, setSearch] = useState('');
  const [dialogOpen, setDialogOpen] = useState(false);
  const [editingRecord, setEditingRecord] = useState<Fahrschueler | null>(null);
  const [deleteTarget, setDeleteTarget] = useState<Fahrschueler | null>(null);
  const [viewingRecord, setViewingRecord] = useState<Fahrschueler | null>(null);
  ...
  async function loadData() {
    setLoading(true);
    try {
      setRecords(await LivingAppsService.getFahrschueler());
    } finally {
      setLoading(false);
    }
  }

  async function handleCreate(fields: Fahrschueler['fields']) {
    await LivingAppsService.createFahrschuelerEntry(fields);
    await loadData();
    setDialogOpen(false);
  }

  async function handleUpdate(fields: Fahrschueler['fields']) {
    if (!editingRecord) return;
    await LivingAppsService.updateFahrschuelerEntry(editingRecord.record_id, fields);
    await loadData();
  ...
  async function handleDelete() {
    ...
    await LivingAppsService.deleteFahrschuelerEntry(deleteTarget.record_id);
  ...
  return (
    <PageShell
      title="Fahrschüler"
      subtitle={`${records.length} Fahrschüler im System`}
      action={
        ...
        <Table className="[&_tbody_td]:px-6 [&_tbody_td]:py-2 [&_tbody_td]:text-base [&_tbody_td]:font-medium [&_tbody_tr:first-child_td]:pt-6 [&_tbody_tr:last-child_td]:pb-10">
          <TableHeader className="bg-secondary">
            <TableRow className="border-b border-input">
              ...
              <TableHead className="uppercase text-xs font-semibold text-secondary-foreground tracking-wider px-6 cursor-pointer select-none hover:text-foreground transition-colors" onClick={() => toggleSort('theorie_bestanden')}>
                <span className="inline-flex items-center gap-1">
                  Theorie bestanden
                  {sortKey === 'theorie_bestanden' ? (sortDir === 'asc' ? <IconArrowUp size={14} /> : <IconArrowDown size={14} />) : <IconArrowsUpDown size={14} className="opacity-30" />}
                </span>
              </TableHead>
          ...
          <TableBody>
            {sortRecords(filtered).map(record => (
              <TableRow key={record.record_id} className="hover:bg-muted/50 transition-colors cursor-pointer" onClick={(e) => { if ((e.target as HTMLElement).closest('button, [role="checkbox"]')) return; setViewingRecord(record); }}>
                ...
                <TableCell><span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-medium ${record.fields.theorie_bestanden ? 'bg-primary/10 text-primary' : 'bg-muted text-muted-foreground'}`}>{record.fields.theorie_bestanden ? 'Ja' : 'Nein'}</span></TableCell>
                <TableCell>
            ...
            {filtered.length === 0 && (
              <TableRow>
                <TableCell colSpan={9} className="text-center py-16 text-muted-foreground">
                  {search ? 'Keine Ergebnisse gefunden.' : 'Noch keine Fahrschüler. Jetzt hinzufügen!'}
                </TableCell>
              </TableRow>
            )}
          </TableBody>
